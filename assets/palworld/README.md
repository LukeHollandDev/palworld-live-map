...
The map artwork and encounter landmarks in this directory are generated from a locally installed copy of Palworld. The repeatable, read-only workflow is documented in the [Palworld Asset Exporter guide](../../exporter/README.md).
...
Both manifests record the game version, exporter and decoder versions, mappings digest, source PAK digests, and extraction sources. The exporter reads `ProjectVersion` directly from `Pal/Config/DefaultGame.ini`; it is not entered manually.
