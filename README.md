...
[![Decoder license: GPL-3.0-or-later](https://img.shields.io/badge/decoder-GPL--3.0--or--later-blue.svg)](third_party/palworld-save-decode/LICENSES/GPL-3.0-or-later.txt)
...
The Alpha Pal and tower-boss catalogue is generated by the project exporter from an installed copy of Palworld, then bundled so deployments do not require the game-data endpoint or save access for those static locations. Save-backed rosters are optional and use a process-isolated reader with a read-only save mount. See [Save-backed data](docs/save-data.md) for the secure setup and data-source model.
...
cloning this repository and building the application and its native helper;
see [Development](DEVELOPMENT.md#run-from-source).
...
See the complete [component licensing table](LICENSING.md). The Go/web application is [MIT](LICENSE). The separately executed, decoder-only helper is [GPL-3.0-or-later](third_party/palworld-save-decode/LICENSES/GPL-3.0-or-later.txt); its complete corresponding source, build instructions, provenance, modifications, and notices are included under [`third_party/palworld-save-decode`](third_party/palworld-save-decode). Every container also carries that exact source under `/usr/src/palworld-save-decode`.

The save reader retains its [Apache-2.0 license](internal/savegame/LICENSE) and [attribution notice](internal/savegame/NOTICE). The encounter catalogue's extraction sources and reproducible workflow are documented alongside the [generated manifest](assets/landmarks/README.md). Corresponding third-party license texts are included under `/licenses` in the container image. The GPL component contains no proprietary Epic or RAD Game Tools Oodle runtime or source code. This project is not affiliated with or endorsed by Epic Games, RAD Game Tools, or Pocketpair.
